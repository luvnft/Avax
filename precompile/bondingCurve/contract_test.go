...
var (
	tests = map[string]testutils.PrecompileTest{
		"insufficient gas for getCost should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
				testInput := GetCostInput{}
				input, err := PackGetCost(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: GetCostGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
		"insufficient gas for getPrice should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				...
				input, err := PackGetPrice(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: GetPriceGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
	}
...
// TestBondingCurvePrecompileRun tests the Run function of the precompile contract.
func TestBondingCurvePrecompileRun(t *testing.T) {
	// Run tests.
...
func BenchmarkBondingCurvePrecompile(b *testing.B) {
	// Benchmark tests.
	for name, test := range tests {
